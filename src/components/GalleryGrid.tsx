...
const KAFKA_DESCRIPTIONS = {
  generation: '使用Flux.1基础模型配合卡芙拉Lora生成的角色展示图，展现了卡芙卡在不同场景和姿态下的魅力。图片风格高度还原原作角色特征，包括发型、服装、表情等细节，体现了Lora模型在角色生成方面的优秀效果。',
  training: '展示以Flux.1为底膜的卡芙拉Lora模型的训练成果，包括不同训练阶段的效果对比、参数调整对生成结果的影响，以及模型在保持角色特征一致性的表现。这些图片直观呈现了Lora训练的技术细节和最终达到的还原度。'
};
